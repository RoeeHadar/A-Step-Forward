...
- **Stream B — Assessment tiers & anti-gaming.** ✅ **Core shipped 2026-07-19.**
  ...
  **Shipped:** gate **retake item rotation** (anti-gaming #6/#7 within-fixed): `weekly_quizzes_ai` gains a `rotation` dimension = prior gate-attempt count, so retakes present a fresh bank slice while reloads within a rotation stay cached. Full **mock exams are now archived** into `test_attempts` (`kind='mock_exam'`, `MOCK_PASS_THRESHOLD=0.6` on auto-graded MCQ) — unifying "all assessments archived" and feeding the readiness mock-gate. Tests archive UI is now **kind-aware** (weekly gate / mock / milestone labels) and renders open (non-MCQ) items gracefully.
  **Shipped (integrity fix):** weekly gates are **authored-bank-first** via `gate-question-bank.ts` — hard/medium `open` / `numeric` / `short_answer` / `derivation` from `lesson_questions` (bundle), recognition MCQ capped ≤25%, easy MCQ skipped. Legacy easy LLM-MCQ caches are ignored (`format_version=2`). Open/derivation answers are LLM-rubric graded on submit (**fail-closed**). LLM only fills concepts with no bank coverage, and only as hard open/numeric (MCQ forbidden).
  **Shipped (Bagrut depth):** `exam-style-corpus` (~100 original multi-part items + mock-paper extracts) is now the **primary** gate/mock source. Weekly gates prefer 4 deep multi-part items (~18 min each). Custom quizzes require `difficulty=hard` and ground on corpus exemplars. LLM mock exams for HS Bagrut are open-only (no MCQ quota); corpus composition preferred; legacy MCQ-heavy mock caches discarded. Official MoE PDFs remain style-reference only — never transcribed.
  **Deferred:** a dedicated milestone/unit-test generator + placement, full Reviewer-agent open-response pipeline (current gate uses a lightweight LLM rubric judge), and between-assessment difficulty auto-calibration beyond the existing per-concept mastery steer.
