...
function upsertQuestions(lesson, tag, questions) {
  ...
  for (const q of questions) {
    ...
    lesson.questions.push({
      ...
      ord,
      needs_review: q.needs_review ?? (q.kind === 'open' || q.kind === 'derivation' ? true : q.needs_review),
...
**Prove arithmetic / geometric.** Show $a_{k+1}-a_k$ constant (arithmetic) or $a_{k+1}/a_k$ constant (geometric) for all $k$ in range — or use the closed form and reverse-engineer.`,
  body_he_md: `**סכום N אחרונים.** בסדרה חשבונית, סכום האיברים האחרונים הוא $S_{\\mathrm{last}\\,n}=S_N-S_{N-n}$. בסדרה הנדסית משתמשים באותו רעיון עם סכומים חלקיים הנדסיים — בלי לחשב מחדש מ-$a_1$.
...
**opt_functional.** Maximize/minimize a pure function value: max slope on a curve, max vertical distance between $f$ and $g$, max $|f(x)-L|$.`,
  body_he_md: `**opt_geometry — אופטימיזציה גאומטרית.** מקסימום או מינימום של שטח, היקף או מרחק באיור (מלבן במשולש, גליל בכדור, מסלול השתקפות). תמיד סקצו תחילה וכתבו אילוץ דמיון או יחס לפני $A'(x)=0$.
