...
class MemoryWriteToolInput(BaseModel):
    """Envelope for the memory.write MCP tool."""
...
class MemoryTimelineInput(BaseModel):
    """Input for memory.timeline MCP tool."""

    learner_id: IDStr
    since: str | None = None
    until: str | None = None
    k: int = Field(default=100, ge=1, le=500)


class MemoryUpdateToolInput(BaseModel):
    """Input for memory.update MCP tool."""
...
class MemoryDeleteToolInput(BaseModel):
    """Input for memory.delete MCP tool."""
...
class LearnerOnlyInput(BaseModel):
    """Single-learner input used by hygiene MCP tools."""
...
class MemoryConsolidateResult(BaseModel):
    """Result of memory.consolidate."""

    merged_or_candidates: int
