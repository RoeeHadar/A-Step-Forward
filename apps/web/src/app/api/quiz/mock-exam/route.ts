...
const VALID_SUBJECTS = new Set(['math', 'physics', 'biology']);
const VALID_LEVELS = new Set(['3pt', '4pt', '5pt', 'hs_physics', 'biology_4pt', 'biology_5pt']);

export async function POST(req: Request) {
  const { userId } = await auth();
  if (!userId) return Response.json({ error: 'Unauthorized' }, { status: 401 });

  let body: { subject?: string; level?: string; duration_minutes?: number };
  try {
  ...
  if (!VALID_SUBJECTS.has(subject)) {
    return Response.json({ error: 'subject must be math, physics, or biology' }, { status: 400 });
  }
  if (!VALID_LEVELS.has(level)) {
    return Response.json(
      { error: 'level must be 3pt, 4pt, 5pt, hs_physics, biology_4pt, or biology_5pt' },
      { status: 400 },
    );
  }
  ...
  try {
    const exam = await getOrCreateMockExam(userId, subject, level, durationMinutes);
    if (!exam) {
      return Response.json(
        {
