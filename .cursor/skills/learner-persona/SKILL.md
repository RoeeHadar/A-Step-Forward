...
description: The CLAUDE.md-style, per-learner persona that EVERY agent reads on every turn. How to read it, how (and when) to write to it, what NEVER to put in it. Pair with `.cursor/skills/chat-memory-context/SKILL.md` (composition order) and `.cursor/skills/agent-skill-notes/SKILL.md` (per-agent private notes).
---
