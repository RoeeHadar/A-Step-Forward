...
5. **`## What I know about this learner (shared persona)`** — the
   ...
   `/api/agent-memory/persona`. See `.cursor/skills/learner-persona/SKILL.md`.
6. **`## My private notes on this learner (agent: <name>)`** — the top-K
   ...
   `/api/agent-memory/notes`. See `.cursor/skills/agent-skill-notes/SKILL.md`.
...
| Agent private notes       | per-(learner, agent)  | `learner_agent_notes`       | Owning agent      | Owning agent (top-K, importance-sorted)     | `.cursor/skills/agent-skill-notes/SKILL.md` |
| Streamed chat turns       | per-(learner, agent, session) | `chat_turns`                | Chat route        | Owning agent (last 4, session-gated)        | This skill                      |
| Mastery                   | per-(learner, concept)| `concept_mastery`           | Grader + lesson/answer routes | Everyone via context              | `.cursor/skills/use-learning-plan/SKILL.md` |
| Atom mastery              | per-(learner, atom)   | `skill_practice`            | Lesson/answer route | Learning planner                          | `.cursor/skills/cross-subject-kg/SKILL.md` |
