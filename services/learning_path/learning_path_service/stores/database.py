...
def get_engine(settings: LearningPathSettings | None = None):
    global _engine, _session_factory
    if _engine is None:
        cfg = settings or LearningPathSettings()
...
def get_session_factory(settings: LearningPathSettings | None = None) -> async_sessionmaker[AsyncSession]:
    get_engine(settings)
...
@asynccontextmanager
async def session_scope(settings: LearningPathSettings | None = None) -> AsyncIterator[AsyncSession]:
